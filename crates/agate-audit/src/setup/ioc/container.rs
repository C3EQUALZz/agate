...
use super::providers::{dispatch_providers, handler_providers, infrastructure_providers};

/// Build the IoC container, started at the App scope. Open a Request scope per
/// request with `container.clone().enter_build()` (the `froodi` axum layer does
/// this per HTTP request).
#[must_use]
pub fn build_container(pool: PgPool) -> Container {
    let ioc = async_registry! {
        extend(
            infrastructure_providers(pool),
            handler_providers(),
            dispatch_providers(),
        )
    };
