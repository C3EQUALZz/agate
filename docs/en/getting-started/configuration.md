...
| `max_response_events` | no | `100000` | Per-run cap on response events streamed to the client. A runaway/hostile agent over this is cut off with a `RUN_ERROR`. `0` = unlimited — a streamed counter, not a buffer, so `0` disables the client-flood (availability) guard, **not** a memory bound (per-run memory is bounded by `max_frame_bytes`). |
...
| `max_response_bytes` | no | `67108864` | Per-run cap on response bytes streamed to the client (64 MiB). `0` = unlimited. Like `max_response_events`, a streamed counter rather than a buffer the proxy fills — `0` is an availability trade-off, not a memory one. |
