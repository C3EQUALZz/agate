...
- `InspectedAction` — вход: вызов инструмента, фрагмент выводимого текста,
  результат инструмента или мутация состояния.
...
- `ArgumentRule` — запрещает разрешённый вызов инструмента, если его аргументы
  совпадают с `Pattern` (опционально с привязкой к одному инструменту);
  применяется `ArgumentInspector` после авторизации имени.
- `ToolName`, `Pattern` — валидируемые объекты-значения (пустая или неверная
  запись отвергается при конструировании). `Pattern` — общий сопоставитель
  контента (литерал ASCII без учёта регистра или regex), используемый и
  редакцией секретов, и правилами по аргументам.
