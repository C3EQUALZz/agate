...
- **Память реплея между прогонами:** по умолчанию выключена; включите
  ...
  stateless-политики. Ledger — в пределах процесса со скользящим TTL по
  умолчанию, либо **Redis** (`backend = "redis"`), общий для реплик и
  переживающий рестарт; Redis-backend fail-open — недоступный Redis
  деградирует до «нет памяти», никогда не wrong-allow.
