...
- **Зависьте от абстракций, а не от реализаций.** Порты — это трейты; конкретные
...
обеспечивает слоистость. Доменные инварианты покрываются **proptest** (например,
round-trip доказательств Меркла, отклонение подделки).
