...
| `[[policy.tools.deny_arguments]]` | таблицы `{ tool?, contains \| matches }` | Правила запрета по аргументам: разрешённый вызов инструмента **блокируется**, если его аргументы совпадают. Каждое правило задаёт ровно одно из `contains` (литерал без учёта регистра) или `matches` (regex). `tool` ограничивает правило одним инструментом; опустите его — правило применится к любому. |
