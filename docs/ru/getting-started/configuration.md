...
предпочитайте атомарную запись (во временный файл, затем rename). Горячая
перезагрузка только под Unix; на других платформах политика фиксируется при старте.
